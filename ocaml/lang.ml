...
=fun e ->
  ...
  | OR (e1, CLOSURE e2) when e1 = e2 -> CLOSURE (opt e1)                (* e|e* -> e* *)
  | OR (CLOSURE e1, e2) when e1 = e2 -> CLOSURE (opt e1)                (* e*|e -> e* *)
  | OR (e1, OR (e2, OZ e3)) when e1 = e3 -> OR (opt e2, OZ (opt e1))    (* e1|(e2|e1?) -> e2|e1? *)
  | OR (e1, OR (OZ e2, e3)) when e1 = e2 -> OR (OZ (opt e1), opt e3)    (* e1|(e1?|e2) -> e1?|e2 *)
  | OR (OZ e1, OR (e2, e3)) when e1 = e2 -> OR (OZ (opt e1), opt e3)    (* e1?|(e1|e2) -> e1?|e2 *)
  | OR (e1, OR (e2, CLOSURE e3)) when e1 = e3 -> OR (opt e2, CLOSURE (opt e1))  (* e1|(e2|e1* ) -> e2|e1* *)
  | OR (e1, OR (CLOSURE e2, e3)) when e1 = e2 -> OR (CLOSURE (opt e1), opt e3)  (* e1|(e1*|e2) -> e1*|e2 *)
  | OR (CLOSURE e1, OR (e2, e3)) when e1 = e2 -> OR (CLOSURE (opt e1), opt e3)  (* e1*|(e1|e2) -> e1*|e2 *)
  | OR (e1, OR (e3, e2)) when e1 = e3 -> OR (opt e1, opt e2)            (* e1|(e1|e2) -> e1|e2 *)
  | OR (e1, OR (e2, e3)) when e1 = e3 -> OR (opt e1, opt e2)            (* e1|(e2|e1) -> e1|e2 *)
  ...
  | CONCAT (CLOSURE e1, CONCAT (CLOSURE e3, e2)) when e1 = e3 -> CONCAT (CLOSURE (opt e1), opt e2) (* e1*(e1*e2) -> e1*e2 *)
  | CONCAT (OZ e1, CONCAT (CLOSURE e3, e2)) when e1 = e3 -> CONCAT (CLOSURE (opt e1), opt e2)      (* e1?(e1*e2) -> e1*e2 *)
  | CONCAT (CLOSURE e1, CONCAT (OZ e3, e2)) when e1 = e3 -> CONCAT (CLOSURE (opt e1), opt e2)      (* e1*(e1?e2) -> e1*e2 *)
  | OR (e1, e2) when e1=ALPHA B && e2=ALPHA A -> OR (opt e2, opt e1)    (* b|a -> a|b *)
